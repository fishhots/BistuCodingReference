...
public class StarServiceImpl implements StarService {
	...
	public StarList getVMovedStars(StarList clearStars,
		...
		StarsUtil.reorder(clearStars);
		int starPosition = clearStars.get(clearStars.size()-1).getPosition().getRow();
		int span = 0;
		...
		for (int row = starPosition; row >= 0; row--) {
			...
			mStar.setMovedPosition(new Position(mStar.getPosition().getRow()+span, column));
